%%
clearvars -except ran, clc, close all, home
...
randRotations   = rotation.rand(1000);
len             = length(randRotations)
...
for i = 1:len
    ...
    % print loop progress
    count = append('progress: ',num2str(i),'/',num2str(length(randRotations)));
    clc;disp('random');disp(count);

    % rotation of selected slice, save rotation for future reference
    rot             = rotation.byEuler(randRotations.phi1(i),randRotations.Phi(i),randRotations.phi2(i));
    rotationSave    = [rotationSave; [rot.phi1 rot.Phi rot.phi2]/degree];
    
    % reference frame to rotate for grain calculation
    rotReference    = rotation.byEuler(-randRotations.phi2(i), -randRotations.Phi(i), -randRotations.phi1(i));
   
    % rotate empty grid, move to center of cuboid
    newGrid     = rotate(plane,rot);
    ...
    ebsdSlice   = rotate(ebsdSlice,rotReference,'keepEuler');
    ...
    [grains, ebsdSlice.grainId] = calcGrains(ebsdSlice,'angle',highAngle);
    ebsdSlice(grains(grains.grainSize <= grainRemove)) = 'notIndexed';

    % calculate grains and subgrains, smooth grain boundaries
    [grains, ebsdSlice.grainId] = calcGrains(ebsdSlice,'angle',[lowAngle, highAngle]);
...
writetable(results,'output\results_IN718_random.csv')
disp('results saved')
...
% save all results in .csv file
writetable(resultsSub,'output\results_IN718_random_subgrains.csv')

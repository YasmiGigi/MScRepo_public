...
% make grid of equispaced point on sphere
gridEquispaced  = equispacedS2Grid('resolution',5*degree);
len             = length(gridEquispaced);
...
for i = 1:len
    ...
    % print loop progress
    count = append('progress: ',num2str(i),'/',num2str(len));
    clc;disp('equispaced');disp(count);

    % rotation of selected slice, save rotation for future reference
    rot             = rotation.map(zvector,gridEquispaced(i));
    rotationSave    = [rotationSave; [rot.phi1 rot.Phi rot.phi2]/degree];

    % reference frame to rotate for grain calculation
    rotReference    = rotation.byEuler(-rot.phi2, -rot.Phi, -rot.phi1);
...
writetable(results,'output\results_IN718_equispaced.csv')
disp('results saved')
...
% save all results in .csv file
writetable(resultsSub,'output\results_IN718_equispaced_subgrains.csv')
